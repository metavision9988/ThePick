...
> 시스템 셧다운 재시작 후 (1) **크래시 무결성 실측 = 손상 0**(git 상태 문서 정합·미커밋 4파일 truncation 0·코드 변경 0·worktree 정상). (2) **진산 "순서대로 진행" 3-스텝 집행**: ①커밋 `d81699b`(W4 canonical form 카드+M1 plan+리뷰 2종+CLAUDE.md 갱신·갱신2, push 보류 #14) ②자율 = **W3 Revision Watch L3 plan** `docs/plans/revision-watch.plan.md` 작성·커밋 `bd57a31` / **W2 2호 spike = 자료 부재로 차단 보고**(exams/·docs/manual-electrical 미존재 = 진산 제공 기출·복원·KEC hwp 대기) ③결재 digest 현행 통합 큐 제시. (3) **W3 plan 근거 = 5축 자산 실사 workflow**(survey 5에이전트·585K tok·0에러): reflect 원시자산(INSERT+SUPERSEDES+draft-only 트리거)·cron 인프라(단일 가드→다중화 필요) 재사용 가능 / **effective_date/시행시점 축 전무**(전 테이블 grep 0)·★**트리거 지뢰**(mav_supersedes가 draft SUPERSEDES INSERT 즉시 approved 노드 비활성화 = AI draft-only 정면충돌)·**alert 채널 0**(ADR-043) 진단. (4) **독립 리뷰 2렌즈+발견별 적대검증 10에이전트 = 8 findings/8 CONFIRMED/0 반증**(MAJOR 3·MINOR 5) 전건 rev2 반영: ★G-RW-4 A안 불완전(status 스냅샷 동결[0018/0013]+승격은 엣지 재INSERT 아님 → **승격시 flip 동반 트리거 필수**)·"단일 필터점" 거짓(study/routes enrichRelatedNodes·vectorize 미시행 노드 누출)·C-3↔B안 트리거 충돌. 보고서 `review-20260705-170953-w3-revision-watch-plan.md`. **plan §9 Q1~Q8 진산 결재 대기**(코드·SQL 착수=결재 후·자율 금지, RW 마이그 슬롯 0041+). ⚠️ origin 대비 **4커밋 ahead**(push=#14 보류). **다음**: 진산 결재(W3 §9 / W4 §8 / M1 §11) / W2=2호 자료 인입 후 / 1호 E0-8 P4~P6·G-S5 0b(검수 후). 상세 [[project_engine_separation_review_20260704]].
> ★ **07-05 후속 — 감지 feasibility spike 실측**(W3 §8 Phase 3a, `docs/feasibility/spike-revision-watch-detection.md`, 라이브 curl+리서치 wf): W3 ④ 감지 = 🔻 미측정 → **🟡 실측 조건부 GO** — **law.go.kr DRF OpenAPI**(법령 target=law + 행정규칙/고시 target=admrul)가 **`시행일자` 구조화 필드** 제공(diffable·발령번호+시행일자 버전키·신구법비교 API) = ★**effective_date를 소스에서 직접 채움 = G-RW-1 근본 해소 경로**. 2호 KEC/기술기준=동일 admrul🟢 / 큐넷 출제기준=HTML+HWP🟡. **신규 진산 액션 = OC 키 무료 발급**(open.law.go.kr) + ★**OC IP-바인딩 ↔ Workers 동적 egress 충돌**(감지 호스트=도메인등록/고정IP 보정 — 순수 Worker cron 불충분). plan rev3(`bd57a31`후 갱신) + §9 Q4 반영. 코드 = OC 발급·§9 결재 후.
