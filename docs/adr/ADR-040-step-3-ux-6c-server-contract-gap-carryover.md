...
- ☐ **M1 — cache key broad** (surgeon Pass 1) — `hashFiles('apps/web/package.json')`이 Playwright 무관 devDep 변경에도 invalidate. `restore-keys: playwright-${{ runner.os }}-` partial fallback 추가 시 평균 50% 다운로드 절감. 비용 최적화, 회귀 무관.
