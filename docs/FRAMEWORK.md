...
| T6 AutoVerify Gate | V1 결정론 대조 / V2 독립 AI 솔버 / V3 검증셋 회귀 — draft 차단 필터. 🔄 2026-07-14 개정: 승격=인간 → **GT 계층별 조건부 자동 승격**(Class A 기출·산식=결정론 대조 / B 원문 대조=캘리브레이션 후 / C 생성물=인간 유지), 인간=표본 감사자. 정본 `docs/plans/decision-card-20260714-autoverify-conditional-promotion.md` | `packages/autoverify/` (신설 예정 — 별건 G-1 선행)                                                                                                               | GT(기출 정답·산식)는 종목 팩이 주입                                            |
