...
```
입력: 기출 PDF (제5~11회, 1차 3과목) ← v1.1 정정: 실제 확보 7회분
출력: { id, year, round, subject, q_no, stem, choices[], answer, explanation? }[]
검증: 1차: 과목당 25 × 7회 = 175, 전체 ~581문항 카운트 + 정답 전수 대조
...
> **v1.1 조정:** 산식 카드만 MVP 범위. 플로우차트·비교 매트릭스·조건트리는 Post-MVP (Won't Do §10-1)

| 항목       | 상세                                                                                                                                                     |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **작업**   | **MVP: 인터랙티브 산식 카드** + 개정 배너. ~~조사 절차 SVG, 조건 분기 아코디언, 비교표~~ → Post-MVP                                                      |
| **산출물** | `formula-card-generator.ts`, `revision-banner.ts` (MVP) / ~~`flowchart-generator.ts`, `condition-tree-generator.ts`, `comparison-matrix.ts`~~ (Post-MVP) |
...
> **v1.1 조정:** 경량 Visualizer + Constants 검수만 MVP. CMS 고급 기능(파이프라인 관리, 사용자 통계)은 Post-MVP (Won't Do §10-1)

| 항목       | 상세                                                                               |
| ---------- | ---------------------------------------------------------------------------------- |
| **작업**   | **MVP: 경량 Graph Visualizer + Constants 검수 UI.** ~~파이프라인 관리~~ → Post-MVP |
| **산출물** | `apps/admin-web/` 확장 (MVP 범위만)                                                |
...
~581문항(7회분) 전수 E2E — 기출 조회 → 풀이 → 해설 → 정답 일치. 불일치 0건 (Hard Stop #1)
