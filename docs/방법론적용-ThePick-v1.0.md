# 방법론 적용 — ThePick v1.2 (effective 2026-04-30)

**작성일:** 2026-04-29 (v1.0/v1.1) / 2026-04-30 (v1.2 — 진산님 §7 결정 영속화 + ADR-027 신설)
...
**v1.1 → v1.2 변경 요지 (2026-04-30):** 진산님 "권고 진행" 응답 처리 — §7 결정 1/2/3 영속화 + handoff-019 §3 결정 4 (Step 11.6 AC-R1 mid-pipeline resume) 추가 영속화. ADR-027 (Year 1 BATCH = atomic, mid-resume Year 2 이연) 신설로 본 v1.2 effective 직후 BATCH-1 적재 진입 차단 게이트 해소. §10.6 changelog 참조.
...
## 7. 진산님 결정 요청 → 응답 영속화 (v1.2 effective 2026-04-30)

본 문서 v1.2 effective. 진산님 "권고 진행" (2026-04-30) 응답 처리 결과:

### 결정 1 — 본 문서 §3 매트릭스 + §2.5 정량 증거 승인 ✅

**채택:** **"승인 — §3 + §2.5 그대로"** (진산 2026-04-30 "권고 진행")

→ 본 문서 v1.2 effective. §3 ✅ 7건 / 🟡 4건 / 🔴 5건 분류 + §2.5 증거 표 (적용 60% / 35% / 5%) 승인 확정.

### 결정 2 — 본 문서의 영속 위치 + 우선순위 ✅

**채택:** **"`docs/방법론적용-ThePick-v1.0.md` 유지 (현 위치)"** (진산 2026-04-30 "권고 진행" 디폴트, 메모리 `feedback_no_granular_decisions` 정합 — 지엽 결정 / handoff-018 §2.2 작업 시간 0d 정합)

→ 본 문서 영속 위치 = 현 경로. 파일명 v1.0 유지 (semver minor patch 만 헤더 명시).

### 결정 3 — P1 시점 정의 ✅

**채택:** **"BATCH-1 dry-run 통과 직후 = P1"** (진산 2026-04-30 "권고 진행", §6.4 정합)

→ B1~B4 부분 채택 4건 (§3.2) 의 ~1.5d 비용은 BATCH-1 dry-run 통과 직후 1주 P1 작업 list 에서 흡수.

### 결정 4 — Step 11.6 AC-R1 mid-pipeline resume 처리 (v1.2 추가, handoff-019 §3) ✅

**채택:** **"옵션 A: Year 1 BATCH = atomic, mid-pipeline resume Year 2 Step 11.7 후보 이연"** (진산 2026-04-30 "권고 진행")

→ ADR-027 신설 (`docs/adr/ADR-027-batch-atomic-mid-resume-deferred.md`). Step 11.6 plan §7 AC-R1 본문 정정. 본 v1.2 effective 직후 BATCH-1 적재 진입 차단 게이트 해소 (4-Pass quality CRITICAL 1건 잔존 → 0건).

**근거 메모리:** `feedback_focus_reliability_not_schedule.md` (안정성·신뢰성·항상성 우선) + `feedback_no_granular_decisions.md` (전략 갈림길 vs 지엽 결정).
...
### 10.6 v1.1 → v1.2 변경 종합 (2026-04-30)

| 섹션            |      v1.1       |                                          v1.2 (2026-04-30 effective)                                          |
| :-------------- | :-------------: | :-----------------------------------------------------------------------------------------------------------: |
| 헤더            |      v1.1       |                              v1.2 (effective 2026-04-30) + 작성일 v1.2 표기 추가                              |
| 변경 요지       |  v1.0 → v1.1만  |                                + v1.1 → v1.2 (진산님 §7 결정 영속화 + ADR-027)                                |
| §7 결정 요청    |  3건 응답 대기  |    **응답 영속화** — 결정 1/2/3 모두 채택 + 결정 4 (handoff-019 §3 AC-R1 옵션 A) 추가 → 본 v1.2 effective     |
| ADR (외부 산출) | ADR-026 후보만  | **ADR-027 신설** (Year 1 BATCH = atomic, mid-resume Year 2 이연) → Step 11.6 4-Pass quality CRITICAL 1건 해소 |
| Step 11.6 plan  | AC-R1 v1.1 본문 |                    AC-R1 본문 정정 (atomic 정책 + already_completed Idempotency skip 검증)                    |

**v1.2 paralysis 신호 점검 (§6.6 정합):**

- v1.1 → v1.2 변경 분량 = 본 §7 응답 영속화 (1 섹션) + §10.6 changelog (1 섹션) + 헤더 갱신만. 본문 §0~§6 / §8~§9 변경 0건.
- v1.2 effective 후속 갱신 트리거는 §9 그대로 — paralysis 신호 (자기 증식) 차단.
- 다음 v1.x 갱신은 BATCH-1 dry-run 통과 후 P1 진입 시점 (B1~B4 부분 채택 흡수 commit) 또는 외부 cross review 1건 도착 시.
