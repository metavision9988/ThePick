...
#
# ★행 앵커 정정 (독립 리뷰 CRITICAL 처분, 2026-08-10) — 초판은 `부칙`·`[별표` 를 **위치 무관**으로
#   찾아서, 조문 본문 안의 **정상 상호참조**를 "삼켰다"로 오판했다:
#     "⑤ 제2항에 따른 업무정지와 위촉 해지 등의 세부기준은 [별표 3]과 같다."  ← 조문의 일부다
#   법령 조문은 별표를 본문에서 인용하는 것이 정상이므로, 위치 무관 검사는 **정상 조문을 반려**한다
#   (실측: 절단 근본원인 수리 후 LAW-172·179 가 이 오탐으로 걸렸다).
#   혼입은 별표·부칙이 **문서 머리로서 행 시작에 오는 것**이고, 그것이 추출기 경계 정의와도 같다.
#   ★탐지력 무손실 확인: LAW-183 사고(별표 산식표 전문 + 부칙 통삼킴)는 그 머리가 행 시작이라
#   행 앵커로도 그대로 잡힌다. 아래 MAX_LEN·DESC_RATIO_MAX 가 2·3차 방어선으로 남는다.
TAIL_CONTAMINANTS = [
    (r"(?m)^\s*부\s*칙", "부칙 혼입"),
    (r"(?m)^\s*\[\s*별\s*표", "별표 혼입"),
