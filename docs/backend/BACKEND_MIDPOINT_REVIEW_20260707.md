...
> 🔄 **2026-07-10 개정 블록 (promo-1st P0~P5 완료 — 원문 보존, 아래 본문 수치는 07-07 시점)**
>
> 1. ★**1차 무료·무인증 홍보 서비스 라이브**: `thepick-study.pages.dev`(랜딩+`/practice/` — 4지선다·빵꾸노트·카드플립·기출 지도·스트릭·결과 공유) + `/api/public/*` 4 라우트(next/grade/reveal/overview) production 개통. 스모크 14/14 + 라이브 브라우저 스모크 PASS. 원장 = `docs/plans/promo-1st-p4-frontend-ledger.md`.
> 2. **§10-B 갭 ① 해소**: 4지선다 보기 추출 BATCH 완료 — **서빙용 -MC 신규 행 521 production 적재**(old 행 무접촉, 0038 트리거 경로). ★동시 **정답 오류 36건 적발·교정**(-MC 행 반영. old 행 36건은 오답 잔존 = `docs/audit/incident-1st-answer-errors-20260710.md`, 처분 마이그 = L3 게이트). ⇒ §10-A "정답 100%" 서술은 **-MC 서빙 행 한정**으로 정정해 읽을 것.
> 3. **진도 관리 신축**: 무인증 로컬 진도 계층(IndexedDB `thepick-local-progress` — FSRS·스트릭·export/import, 서버 user 데이터 0 = G-1). 인증 경로와 별도 축.
> 4. **정답 안전 가드 신설**: 인증 study `/next`·`/grade` 에 MC-in-disguise 서빙·채점 가드(C-1) + 조기 거짓 exhausted 수정(D-02) — old 행 오답 36 이 인증 표면에서도 서빙 불가.
> 5. **운영 안전망**: D1 **오프사이트 백업 신설**(R2 `thepick-backups`, 첫 스냅샷 07-10 + `scripts/backup-d1-to-r2.sh` + 런북 §10 — §8 "백업 부재" 서술 해소) / **CI 장기 red 수리**(parser venv·quality tsx·web 테스트 배선·G-1 게이트 문면 정합) / 배포 후 공개 표면 스모크 스크립트(M-17).
> 6. **§9.5 즉시군 상태 변경**: M2 카드플립·M1 빵꾸노트(+M9 힌트)·M14 스트릭·F9 지형도(MOC) = **공개 표면에 구현·라이브 완료**. ⚠️ 단 M1 서술 "서빙 대상 전량 fill_blank = 대상 전면"은 **역전**: fill_blank 원행은 전량 위치라벨 정답(MC-in-disguise)이라 서빙 자격 0 — 빵꾸노트 라이브 데이터 0(진성 fill_blank 콘텐츠 = 후속 트랙).
> 7. 리뷰 4회(4-Pass ×2 + 5-페르소나 ×2) CRITICAL 누적 4건 전건 즉시 처분. 잔여 이월 원장 = promo-1st 원장 §7·§8.2b.
...
| 갭                         | 실측                                                                                                                                                               | 해소 경로                                                                       |
| :------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------ |
| ① **4지선다 보기 미공급**  | 🔄 **07-10 해소** — -MC 서빙 행 521 production 적재(정답 교정 36 반영)·공개+인증 양표면 MC 서빙 가동. ~~distractors 전량 NULL~~(old 행 한정 잔존 — 서빙 제외 가드) | ~~기출 원문의 ①~④ 보기 추출 BATCH~~ **완료**(P3). 잔여 = 구조훼손 정직 제외 4건 |
...
> 🔄 **07-10**: ① 완료 + 그 위에서 **1차 무료·무인증 서비스가 이미 라이브**(상단 개정 블록). 1차 단독의 잔여 = ② 해설 조달(진산) ③ 근거 코퍼스 검수 승급(진산 — P1~P3 검수) + 공통 related_nodes 백필(Phase C)로 압축.
