...
[완료 게이트] G1: 러너가 합성 픽스처(정상/고아/순환/도달불가 4종)에서 정확 분류 테스트 PASS. G2: ~~CONCEPT-023 기지 양성~~ (2026-06-11 실측 반증 — 유효 inbound 1, 영구 불충족 조건. 대체 = 도달불가 검출기 작동 입증 + 합성 양성 테스트. EXPANSION_GATE_DESIGN E0-2 정정 참조). G3: 덤프 부재 시 명시 에러 테스트. G4: batch 패키지 회귀 0. G5: 4-Pass CRITICAL 0.
...
**완료 (2026-06-11 13:15, 커밋 723908e).** 독립 리뷰 2-agent: **C1(양 리뷰 교차 적중 — distractors 무음 filter 위치 시프트 오채점) + M4 + m12 → 전건 해소** `review-20260611-131500`. 최종: filter→every 전수 거부·5지선다 전순열 게이트 추가·MC_MAX_CHOICES=CHOICE_LABELS 파생·유니코드 적대 거부 영속·결재카드 ☑ 동기. **E0-1 = 코드 측 PASS** (실데이터 검증 = 7b 적재부터). carry-over: 자정 시드 경계(#11 트랙 처분). 검증: api 685/0·lm 125/0·study 85/85.
