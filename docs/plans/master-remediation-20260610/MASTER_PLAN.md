...
| ☑ 17 | **2호 후보 R3 spike 착수 승인**                    | 확장 전략   | **진산 승인 (2026-06-11 "권고대로")** — 후보: 공인중개사/소방/산업안전, PDF 1건 구조화→무결성 PASS 실측(버려질 스파이크·전 게이트 병렬). 전기기사 = 2호 배제(E2-0) → 3호. ⚠️ **[2026-07-04 R5 번복 → 전기기사 = 2호 확정(동시진행). 개정 근거: 표 아래 🔄 개정 블록 + `decision-card-20260704-engine-separation-r5.md`]** **선결: 후보 시험 공개 기출/교재 PDF 확보(진산 공급 또는 출처 지정) 후 착수** **+ (2026-06-11 보충) 자료 확보 = 데드라인 없음·진산 여유 시 — 공개 기출 1회분 1건이면 충분(버려질 스파이크·정확성 검증 불요). 1호 E0 집중이 우선** |
...
> **🔄 2026-07-04 R5 결재 개정 (D-6 문면 정합 — 원문 보존, 삭제 금지):**
>
> 1. **#17 "전기기사 = 2호 배제 → 3호" 번복** — 2026-07-04 진산 R5 결재로 **2호 = 전기기사 확정**(동시진행, git worktree 이중 워크스페이스). 배제 사유였던 "복소수 타입 계약 재설계·Vision" 기술 사실은 유효하나, 그 역할 배정(2호/3호)이 사업 결재로 재지정됨(확장 급함 → rule-of-three 대기 해제 = 단일 레포 `exams/` 분리 A안 즉시 착수 궤도). 근거: `docs/plans/decision-card-20260704-engine-separation-r5.md` + `docs/feasibility/exam2-electrical.feasibility.md` R5 + 플레이북 `docs/plans/opus-dual-track-playbook-20260704.md` W1.
> 2. **§2.3 "Year 2 멀티시험 실전환 = 이월 유지" 부분 개정** — 확장 단위(모노레포 `exams/{id}` 분리 + 통합 계정 플랫폼 D1)는 A안으로 **조기집행**(ADR-007 2026-07-04 Amend). 단 1호 기존 9테이블 in-place `exam_id` 소급(구 마이그 0005 breaking) + 레포 밖 프레임워크 추출(B안)은 **여전히 이연**(supersede 아님). 상세 = ADR-007 Amendment + decision-card §2.
> 3. **#16 E0~E4 게이트 채택은 유효** — E2-0/E3-1 선정·순서 규칙만 위 1·2로 개정(EXPANSION_GATE_DESIGN 상단 🔄 개정 배너 참조). E0~E1 1호 게이트·R3 spike 병렬 원칙은 불변.
>
