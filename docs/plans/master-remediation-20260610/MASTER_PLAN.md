...
- 2c: QG-2 드리프트 수정 — 헤더 주석을 코드 정본(40/80/7)에 동기 + batchId 배선 + BATCH-6+ 누적 임계 정의 + (옵션) D1 적재물 카운트 모드 (`qg2-validator.ts:4-8,96-105,216`) [L2]
...
**Binary Gate G-WS2**: ① 러너가 2026-05-15 실측 스냅샷(794/1274) 또는 로컬 재현 덤프에서 고아 0·끊긴엣지 0·순환 0·도달불가 노드 리스트 출력 ② CONCEPT-023이 도달불가 리스트에 검출됨(기지 양성 대조) ③ 가드 마이그: UPDATE 시도 ABORT + is_active 플립 허용 테스트 (G-TR0 패턴) ④ ADR-014 상태 변경 기록.
...
| ☐ 2  | MC 채점 answer 계약 (1a)                                    | WS-1 | 라벨형('③') / 텍스트형 / 조사 후 재상신                                                                                                       |
...
| ☐ 16 | **E0~E4 확장 게이트 채택 + 하한값 4곳 확정** | 확장 전략 | EXPANSION_GATE_DESIGN §게이트 설계안 — E0-4 hit-rate 하한 / E1-3 근거보기 비율 / E1-5 유료전환 포함 여부 / E4-4 표본 N. 게이트 문구 개정 = L3급 결재(침식 차단)                                                                                                                                                                          |
| ☐ 17 | **2호 후보 R3 spike 착수 승인**              | 확장 전략 | 전 게이트와 병렬 허용(검증 H5 — "동시 다발" 논거의 유일 안전 적용처). 후보: 공인중개사/소방/산업안전 중 — PDF 1건 구조화→무결성 PASS 실측, 버려질 스파이크. 전기기사 = 2호 배제(E2-0: 코어 수정 사전 확정 = QG-M2 정의상 모순) → 3호 역할                                                                                                |
