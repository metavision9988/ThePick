...
| 3-UX-6c ✅ 완료 | ModeSelector + SessionStart + SessionSummary       | 6a LOCK + `/api/study/mode` 매핑 layer | `study-api.ts` + `session/types.ts` + `ModeSelector` + `SessionStart` + `SessionSummary` + `StudyFlow.tsx` + study.astro 통합 + ADR-039 (5 mode) + ADR-040 (carry-over) — Session 071 |
...
- [ ] 3-UX-7f — production seed + 4-Pass + 5-페르소나
