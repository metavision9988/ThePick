...
| **Graph-walk multi-hop 정답률 기여** | **혼재 — 도메인·데이터 의존, 이득 비보장** | **🟡 (측정됨, 재측정 필요)** | R3 측정(06-01) → 5-페르소나 감사(06-02): **NO-GO 시기상조**. 측정값 재현되나 graph 유효표본 N=1·regression maxDepth1 가역 → graph 가치 현 데이터로 확정 불가, 재측정 후 판단 |
