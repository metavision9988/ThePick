...
| **multi-hop 정답률 (북극성)** | **G-S5 golden**  | 🟡       | **2차 실측 완료(2026-06-05, queryBody 정화 N=6)**: baseline 83.3% / depth1 무해(Δ0.0%·regression 0) / depth2 순손실(Δ−20%·regression 1) / graphOnlyRecovery 0 both — 06-02 감사 3대 가설 전부 확증 | **graph 현 파라미터 순기여 0 (재설계 미시도 — "사망" 단정은 여전히 비약)** | ❌ (Ground Truth+raw 양일 실측) |
...
- **R3 2차 실측 (2026-06-05, 진산 결재 #2-결재카드 queryBody 분리 후):** 감사가 요구한 재측정 수행 — golden query 를 출제 본문만으로 정화(`golden-pilot-approved.querybody.json`, measurable 4→6·graph 유효 multi-hop 표본 1→3). **결과: ① depth1 = 무해·무익**(hit-rate Δ 0.0%, 83.3→83.3 / regression 0) **② depth2 = 순손실**(Δ −20.0% / regression 1 = Q-012 F-노드 범람) **③ graphOnlyRecovery = 0 both**. 06-02 감사 3대 가설(Q-012=depth2 아티팩트 / N 1→3 / baseline 100%=부분 아티팩트→정화 후 **83.3%가 진실**) 전부 실측 확증. 진짜 graph-only headroom = 2노드(CONCEPT-023·INS-27, 전부 Q-015 단일 문항). **후속 집행(2026-06-11, 결재 #6):** production 기본 maxDepth 2→1 코드 전환(순손실 차단 — `DEFAULT_MAX_DEPTH=1`, S5-8 Phase 0a). 재설계(Phase 1+ hop 감쇠·랭킹 융합) 및 lexical fusion D안 비교군(결재 #7)은 plan 등재 — 측정 후 R5 재상신.
...
- **결정자:** 진산 / **축소 GO 기록일:** ~Phase 3 launch / **graph-walk 전체 GO 결정일:** ⏸️ 조건부 보류 (2026-06-11 결재 #8 — D안 비교 측정 후 재상신. 1차 06-01 + 2차 06-05 실측 완료)
