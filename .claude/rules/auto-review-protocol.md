# 2단계 독립 리뷰 프로토콜 — ThePick 특화

# (1) L2+ 개별 구현 완료 후 → 4-Pass 리뷰 (코드 정합성)

# (2) Phase 마일스톤 완료 후 → 5-페르소나 기술부채 심층 리뷰 (기술부채)

# 두 프로토콜 모두 선택이 아니다. 스킵하면 CRITICAL RULE #4 위반이다.

---

## Phase 단위 5-페르소나 기술부채 리뷰 (신규 추가, 2026-04-18)

### 트리거

- Phase 0, 1, 2, 3 각 완료 시점
- 대규모 묶음 변경 완료 시 (예: ADR 5건 + 패키지 신설 + 업그레이드 3종)
- 진산님 "고고" 승인 후 누적 변경이 "하나의 이정표"가 된 시점

### 5개 전문 페르소나 (4-Pass 에이전트와 중복 금지)
...
- 4-Pass 결과와 별도 관리, 우선순위 매트릭스 진산님 보고

### "완료" 선언 기준 (강화)

- 4-Pass CRITICAL 0건 **AND** 5-페르소나 CRITICAL 0건
- MAJOR는 phase 종료 전 해결 또는 다음 phase 초기 태스크로 **명시 이월**

---

## 4-Pass 자동 리뷰 프로토콜 (기존)
